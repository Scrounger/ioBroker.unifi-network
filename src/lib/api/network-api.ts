...
export class NetworkApi extends EventEmitter {
    ...
    private async checkSites(): Promise<boolean> {
    ...
    public async getVpnUser(): Promise<myNetworkClient[] | undefined> {
        const logPrefix = `[${this.logPrefix}.getVpnUser]`

        try {
            const res = await this.retrievData(`${this.getApiEndpoint_V2(ApiEndpoints_V2.vpnUsers)}`);
